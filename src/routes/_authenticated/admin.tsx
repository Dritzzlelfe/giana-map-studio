...
function AdminPage() {
  ...
  if (!isAdmin) {
    return (
      <AppShell>
        <div className="p-6 text-sm text-muted-foreground">You don't have access to this page.</div>
      </AppShell>
    );
  }
  ...
  return (
    <section className="rounded-lg border bg-card p-4">
      ...
      <p className="mb-3 text-xs text-muted-foreground">
        Overrides the UI navigation and permission gates for your current session so you can see
        what a given role would see. Data reads still happen with your real role; use a real invited
        account for a full end-to-end verification.
      </p>
      <div className="flex items-center gap-2">
        <Select
          value={previewKey ?? "__off"}
          onValueChange={(v) => setPreviewKey(v === "__off" ? null : v)}
        >
        ...
        <div className="space-y-3">
          {roles.map((r) => (
            <RoleRow
              key={r.id}
              role={r}
              onSave={(patch) => upsert.mutate({ id: r.id, ...patch })}
              onDelete={() => {
...
function RoleRow({
  ...
  const [rights, setRights] = useState<Record<string, RightLevel>>(() => ({
    ...role.module_rights,
  }));
      ...
      <div className="mb-3 flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex gap-2">
          <Button
            size="sm"
            disabled={!dirty}
            onClick={() => onSave({ label, module_rights: rights, money_visibility: money })}
          >
            Save
          </Button>
          <Button
...
function PeoplePanel() {
  ...
  return (
    <section className="rounded-lg border bg-card p-4">
      ...
      <p className="mb-3 text-xs text-muted-foreground">
        Assign a role to each signed-in user. New users get a profile automatically on their first
        sign-in and start with no role until you assign one. Send them the app URL and ask them to
        sign up with their email — they will then appear here for role assignment.
      </p>
              ...
              <tr key={p.id}>
                ...
                <td className="border-b py-2">
                  <Select
                    ...
                    onValueChange={(v) =>
                      assign.mutate({ id: p.id, role_id: v === "__none" ? null : v })
                    }
                  >
