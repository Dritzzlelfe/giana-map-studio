  ...
  return (
    <AppShell>
      <div className="flex-1 overflow-auto p-6">
        ...
        <p className="mb-4 text-sm text-muted-foreground">Pick a room to see all of its items grouped by category.</p>
        {isLoading && (
          <div className="text-muted-foreground"><Loader2 className="mr-2 inline h-4 w-4 animate-spin" /> Loading…</div>
        )}
        {data && (
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {data.rooms.map((r) => {
              ...
              return (
                ...
                >
                  <span className={r.active ? "font-medium" : "font-medium opacity-60"}>{r.name}</span>
